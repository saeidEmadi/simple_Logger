...
void Logger::handler(QtMsgType type, const QMessageLogContext &context, const QString &msg)
{
    if (Logger::logging)
    {
        ...
        switch (type)
        {
            case QtInfoMsg:
            {
                txt = QString("QtInfoMsg: %1").arg(msg);
                break;
            }

            case QtDebugMsg:
            {
                txt = QString("QtDebugMsg: %1").arg(msg);
                break;
            }

            case QtWarningMsg:
            {
                txt = QString("QtWarningMsg: %1").arg(msg);
                break;
            }

            case QtCriticalMsg:
            {
                txt = QString("QtCriticalMsg: %1").arg(msg);
                break;
            }

            case QtFatalMsg:
            {
                txt = QString("QtFatalMsg: %1").arg(msg);
                break;
            }
        }
